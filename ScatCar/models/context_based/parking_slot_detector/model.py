...
class yolov3(object):
    ...
    def reorg_layer(self, feature_map, anchors, angle):
        ...
        grid_size = (
            feature_map.get_shape().as_list()[1:3]
            if self.use_static_shape
            else tf.shape(feature_map)[1:3]
        )
        # the downscale ratio in height and weight
        ratio = tf.cast(self.img_size / grid_size, tf.float32)

        feature_map = tf.reshape(
            feature_map, [-1, grid_size[0], grid_size[1], 3, 13 + self.class_num]
        )

        # split the feature_map along the last dimension
        box_centers, box_sizes, conf_logits, prob_logits, quad_points = tf.split(
            feature_map, [2, 2, 1, self.class_num, 8], axis=-1
        )
        ...
        rot_mat = tf.stack(
            [
                tf.stack([tf.cos(-angle * math.pi / 180), -tf.sin(-angle * math.pi / 180)], -1),
                tf.stack([tf.sin(-angle * math.pi / 180), tf.cos(-angle * math.pi / 180)], -1),
            ],
            -2,
        )
    ...
    def predict(self, feature_maps, angle):
        ...
        reorg_results = [
            self.reorg_layer(feature_map, anchors, angle)
        ...
        def _reshape(result):
            ...
            grid_size = (
                x_y_offset.get_shape().as_list()[:2]
                if self.use_static_shape
                else tf.shape(x_y_offset)[:2]
            )
    ...
    def loss_layer(self, feature_map_i, y_true, anchors, angle):
        ...
        x_y_offset, pred_conf_logits, pred_prob_logits, pred_quad_points = self.reorg_layer(
            feature_map_i, anchors, angle
        )
        ...
        def loop_body(idx, ignore_mask):
            valid_true_quads = tf.boolean_mask(
                y_true[idx, ..., 5 + self.class_num : 13 + self.class_num],
        ...
        box_loss_scale = 2.0 - (y_true[..., 2:3] / tf.cast(self.img_size[1], tf.float32)) * (
        ...
        quad_loss = (
            tf.reduce_sum(
                (
                    tf.abs(true_p1 - pred_p1)
                    + tf.abs(true_p2 - pred_p2)
                    + tf.abs(true_p3 - pred_p3)
                    + tf.abs(true_p4 - pred_p4)
                )
                * object_mask
                * box_loss_scale
                * mix_w
        ...
        class_loss = (
            object_mask
            * tf.nn.sigmoid_cross_entropy_with_logits(labels=label_target, logits=pred_prob_logits)
            * mix_w
        )
    ...
    def quad_iou(self, pred_quad_points, valid_true_quads):
        ...
        # [13, 13, 3, 2]
        pred_box_xy = (
            pred_quad_points[..., 0:2]
            + pred_quad_points[..., 2:4]
            + pred_quad_points[..., 4:6]
            + pred_quad_points[..., 6:8]
        ) / 4
        pred_box_w = tf.maximum(
            tf.maximum(pred_quad_points[..., 0], pred_quad_points[..., 2]),
            tf.maximum(pred_quad_points[..., 4], pred_quad_points[..., 6]),
        ) - tf.minimum(
            tf.minimum(pred_quad_points[..., 0], pred_quad_points[..., 2]),
            tf.minimum(pred_quad_points[..., 4], pred_quad_points[..., 6]),
        )
        pred_box_h = tf.maximum(
            tf.maximum(pred_quad_points[..., 1], pred_quad_points[..., 3]),
            tf.maximum(pred_quad_points[..., 5], pred_quad_points[..., 7]),
        ) - tf.minimum(
            tf.minimum(pred_quad_points[..., 1], pred_quad_points[..., 3]),
            tf.minimum(pred_quad_points[..., 5], pred_quad_points[..., 7]),
        )
        ...
        # [V, 2]
        true_box_xy = (
            valid_true_quads[:, 0:2]
            + valid_true_quads[:, 2:4]
            + valid_true_quads[:, 4:6]
            + valid_true_quads[:, 6:8]
        ) / 4
        true_box_w = tf.maximum(
            tf.maximum(valid_true_quads[:, 0], valid_true_quads[:, 2]),
            tf.maximum(valid_true_quads[:, 4], valid_true_quads[:, 6]),
        ) - tf.minimum(
            tf.minimum(valid_true_quads[:, 0], valid_true_quads[:, 2]),
            tf.minimum(valid_true_quads[:, 4], valid_true_quads[:, 6]),
        )
        true_box_h = tf.maximum(
            tf.maximum(valid_true_quads[:, 1], valid_true_quads[:, 3]),
            tf.maximum(valid_true_quads[:, 5], valid_true_quads[:, 7]),
        ) - tf.minimum(
            tf.minimum(valid_true_quads[:, 1], valid_true_quads[:, 3]),
    ...
    def compute_loss(self, y_pred, y_true, angle):
        ...
        """
        loss_conf, loss_class, loss_quad = 0.0, 0.0, 0.0
